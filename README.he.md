...
עדיין אין מתקין ייעודי ל-Windows, אבל הליבה חוצת-פלטפורמות: גם whisper.cpp וגם ffmpeg רצים על Windows, ו-[AutoHotkey](https://www.autohotkey.com/) ממלא את אותו התפקיד ש-Hammerspoon ממלא ב-macOS (קיצור מקלדת גלובלי, הקלטה, הדבקה). הדרך המעשית להתקין את זה היום היא דרך סוכן ה-AI שלמטה: מוסרים לסוכן את [PROMPT.md](PROMPT.md), שכולל כעת הנחיות ל-Windows. אם הצלחתם, PR עם תיקיית `windows/` יתקבל בברכה.
